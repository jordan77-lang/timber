...
try {
  ...
  console.error("Fatal Error: Could not append renderer to the container.", e);
  const container = document.getElementById('container');
  if(container) {
    container.innerHTML = `<div style="color: red; border: 1px solid red; padding: 20px; margin: 20px;">
                             <strong>A critical error occurred.</strong><br>
                             Could not initialize the 3D scene. Please check the browser console for details.<br>
                             Error: ${e.message}
                           </div>`;
  }
  // Stop all further execution
  throw new Error("Halting script due to rendering setup failure.");
}
...
// This line is intentionally left blank to remove the duplicate declaration.
const hands = [];
const handModels = [];
